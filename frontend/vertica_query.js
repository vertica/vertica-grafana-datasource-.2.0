...
export default class VerticaQuery {
//   target: any;
//   templateSrv: any;
//   scopedVars: any;
  ...
  hasTimeGroup() {
    return _.find(this.target.group, (g) => g.type === 'time');
  }
  ...
  buildValueColumn(column) {
    ...
    const columnName = _.find(column, (g) => g.type === 'column');
    query = columnName.params[0];

    const aggregate = _.find(column, (g) => g.type === 'aggregate' || g.type === 'percentile');
    const windows = _.find(column, (g) => g.type === 'window' || g.type === 'moving_window');
    ...
    const alias = _.find(column, (g) => g.type === 'alias');
    if (alias) {
