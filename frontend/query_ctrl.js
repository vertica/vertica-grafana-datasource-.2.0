...
import { VerticaMetaQuery } from './meta_query';
...
//   groupAdd: any;
  ...
  updateProjection() {
    this.selectParts = _.map(this.target.select, (parts) => {
  ...
  updatePersistedParts() {
    ...
    this.target.where = _.map(this.whereParts, (part) => {
      return { type: part.def.type, datatype: part.datatype, name: part.name, params: part.params };
    });
    this.target.group = _.map(this.groupParts, (part) => {
      return { type: part.def.type, datatype: part.datatype, params: part.params };
    });
  }
  ...
  getTableSegments() {
    return this.datasource
      .metricFindQuery(this.metaBuilder.getTableQuerySQL(this.schemaSegment.value))
  ...
  getSchemaSegments() {
    return this.datasource
      .metricFindQuery(this.metaBuilder.getSchemaQuerySQL())
      .then(this.transformToSegments({}))
      .catch(this.handleQueryError.bind(this));
  }
  ...
  tableChanged() {
    ...
    const task1 = this.datasource.metricFindQuery(this.metaBuilder.getColumnQuerySQL('time')).then(result => {
      // check if time column is still valid
      if (result.length > 0 && !_.find(result, (r) => r.text === this.target.timeColumn)) {
    ...
    const task2 = this.datasource.metricFindQuery(this.metaBuilder.getColumnQuerySQL('value')).then(result => {
      if (result.length > 0) {
  ...
  getTimeColumnSegments() {
    return this.datasource
      .metricFindQuery(this.metaBuilder.getColumnQuerySQL('time'))
      .then(this.transformToSegments({}))
      .catch(this.handleQueryError.bind(this));
  }
  ...
  getMetricColumnSegments() {
    return this.datasource
      .metricFindQuery(this.metaBuilder.getColumnQuerySQL('metric'))
  ...
  findAggregateIndex(selectParts) {
    return _.findIndex(selectParts, (p) => p.def.type === 'aggregate' || p.def.type === 'percentile');
  }

  findWindowIndex(selectParts) {
    return _.findIndex(selectParts, (p) => p.def.type === 'window' || p.def.type === 'moving_window');
  }
  ...
  addSelectPart(selectParts, item, subItem) {
    ...
    switch (partType) {
      ...
      case 'aggregate':
        ...
        if (!_.find(selectParts, (p) => p.def.type === 'alias')) {
          addAlias = true;
        }
        break;
      case 'moving_window':
    ...
    switch (evt.name) {
      case 'get-param-options': {
        switch (part.def.type) {
          ...
          case 'column':
            return this.datasource
              .metricFindQuery(this.metaBuilder.getColumnQuerySQL('value'))
              .then(this.transformToSegments({}))
              .catch(this.handleQueryError.bind(this));
        }
      }
  ...
  handleGroupPartEvent(part, index, evt) {
    switch (evt.name) {
      case 'get-param-options': {
        return this.datasource
          .metricFindQuery(this.metaBuilder.getColumnQuerySQL())
          .then(this.transformToSegments({}))
          .catch(this.handleQueryError.bind(this));
      }
      case 'part-param-changed': {
  ...
  removeGroup(part, index) {
    if (part.def.type === 'time') {
      // remove aggregations
      this.selectParts = _.map(this.selectParts, (s) => {
  ...
  getGroupOptions() {
    return this.datasource
      .metricFindQuery(this.metaBuilder.getColumnQuerySQL('group'))
      .then(tags => {
        const options = [];
